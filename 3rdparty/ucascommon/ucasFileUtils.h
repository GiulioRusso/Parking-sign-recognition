...
#include <errno.h>
	...
	inline void remove_folder(const std::string & path, bool verbose = false) throw (Error)
	{
		if(verbose)
	...
	inline void rename_file(const std::string & old_path, const std::string & new_path, bool verbose = false) throw (Error)
	{
		if(verbose)
