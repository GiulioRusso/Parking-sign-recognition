...
void ucas::stripewrite(const std::string & path, const std::vector <cv::Mat> & patches, bool normalize) throw (ucas::Error)
{
	// check preconditions
	if(patches.empty())
...
// get histogram image
cv::Mat ucas::imhist(const cv::Mat & image) throw (ucas::Error)
{
