	...
	**************************
	...
	class BinaryClassMetric
	{
		public:
			...
			virtual T evalFromROC( const std::vector < ROCpoint<T> > & ROC ) throw (ucas::Error) = 0;

			// factory method
			static BinaryClassMetric<T>* instance(const std::string & str) throw (ucas::Error);
	...
	class AUC_Metric : public BinaryClassMetric<T>
	{
		public:
			...
			virtual T evalFromSamples(const std::vector <T> & pos, const std::vector <T> & neg, bool pos_greater_than_neg = true, T threshold = 0) throw (ucas::Error)
			{
				return ucas::AUC_trapz<T>(pos, neg, 1, pos_greater_than_neg);
			}

			virtual T evalFromROC( const std::vector < ROCpoint<T> > & ROC ) throw (ucas::Error)
			{
	...
	class logPAUC_Metric : public BinaryClassMetric<T>
	{
		...
		public:
			...
			logPAUC_Metric(const std::string & str) throw (Error)
			{
	...
	class PAUC_Metric : public BinaryClassMetric<T>
	{
		...
		public:
			...
			PAUC_Metric(const std::string & str) throw (Error)
			{
				std::string args = str.substr(str.find_first_of("(")+1);
				args = args.substr(0, args.find_last_of(")"));
	...
	template <typename T>
	BinaryClassMetric<T>* BinaryClassMetric<T>::instance(const std::string & str) throw (ucas::Error)
	{
