...
namespace ucas
{
	...
	// get histogram image
	cv::Mat imhist(const cv::Mat & image) throw (ucas::Error);
	...
	void    imwrite(const std::string & path, cv::Mat & img) throw (ucas::Error);

	// write a horizontal stripe of patches
	void    stripewrite(const std::string & path, const std::vector <cv::Mat> & patches, bool normalize = false) throw (ucas::Error);
	...
	cv::Mat heatMap(const cv::Mat & float_mat, bool normalize, bool invert) throw (ucas::Error);
}
...
namespace ucas
{
	...
	class Blob
	{
		...
		public:
			...
			Blob(std::vector<size_t> & shape, bool allocate_memory = true, T init_val = 0) throw (Error) 
			{
				_type = typeid(T).name();
				_shape = shape;
				_data = 0;
			...
			void setShape(size_t at, size_t val) throw (Error)
			{
				if(at<_shape.size())
					_shape[at] = val;
				else
