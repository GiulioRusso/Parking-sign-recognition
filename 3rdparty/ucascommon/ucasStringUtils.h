...
namespace ucas
{
	...
	inline const char* cpadding(const std::string & str, size_t fixed_dim, char c = ' ')
	{
		return padding(str, fixed_dim, c).c_str();
	}
	...
	inline static void parse_range(const std::string & str, int &a, int &b, int &c, int &d) throw (Error)
	{
