...
namespace aia
{
	...
	********************************************
	...
	throw (aia::error)
	{
		...
		if (!capture.isOpened())
			throw aia::error(aia::strprintf("Cannot open input video stream from %s", inputPath.empty() ? "camera" : inputPath.c_str()));
		...
		if(!outputPath.empty())
		{
			...
			if (!output.isOpened())
				throw aia::error("Cannot open output video stream");
		}
		...
		while (!stop) 
		{
			...
			//cv::resize(processedFrame, processedFrame, cv::Size(0,0), 0.5, 0.5);
	...
	inline std::vector < cv::Rect > faceDetector(const cv::Mat& frame) throw (aia::error)
	{
		if(!aia::isFile(FACE_DETECTOR_PATH))
			throw aia::error(aia::strprintf("Cannot load face detector from \"%s\"", FACE_DETECTOR_PATH.c_str()));
